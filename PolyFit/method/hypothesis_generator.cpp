...
bool HypothesisGenerator::do_intersect(MapTypes::Facet* f, MapTypes::Facet* plane)
{
    ...
    facet_attrib_supporting_vertex_group_[f]->point_set();
...
void HypothesisGenerator::pairwise_cut(Map* mesh)
{
	...
	for (std::size_t i = 0; i < all_faces.size(); ++i) {
		...
		//TODO: Change to cut less mesh, to reduce computation and more accuracy.
        std::cout<<"intersecting_faces"<<intersecting_faces.size()<<std::endl;
